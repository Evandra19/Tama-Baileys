        ...
        try {
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    if (msg) {
                        const fromMe = (0, WABinary_1.areJidsSameUser)(node.attrs.participant || remoteJid, authState.creds.me.id) || (!!authState.creds.me.lid && (0, WABinary_1.areJidsSameUser)(node.attrs.participant || remoteJid, authState.creds.me.lid));
                        msg.key = {
                            remoteJid,
                            fromMe,
